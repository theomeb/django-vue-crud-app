...
export default {
  ...
  methods: {
    getData(table) {
      const path = ('http://localhost:8000/apiemoeth/').concat(table.toLowerCase()).concat('/?format=json');
      axios.get(path)
        .then((res) => {
          this.items = res.data;
        })
    ...
    deleteItem(item) {
      ...
      const path = ('http://localhost:8000/apiemoeth/delete/?table=').concat(tableName).concat('&name=').concat(name);
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        ...
        let path = ('http://localhost:8000/apiemoeth/edit/?table=').concat(tableName).concat('&name_to_changed=').concat(nameToChanged).concat('&name=')
          .concat(name)
          .concat('&total_docs=')
          .concat(totalDocs);
        ...
        const name = this.editedItem.name;
        const totalDocs = this.editedItem.total_docs;
        let path = ('http://localhost:8000/apiemoeth/create/?table=').concat(tableName).concat('&name=').concat(name).concat('&total_docs=')
          .concat(totalDocs);
    ...
    init() {
      const path = ('http://localhost:8000/apiemoeth/init');
      this.triggerBase(path);
    },
  },
  created() {
